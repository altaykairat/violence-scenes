...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, bottleneck_channels, out_channels, downsample=None, stride=1, n_segments=8):
        super(Bottleneck, self).__init__()

        self.conv1 = nn.Conv2d(
            in_channels=in_channels, out_channels=bottleneck_channels, kernel_size=1, bias=False)
        self.conv1 = TSM(self.conv1, n_segments=n_segments, n_div=8)
        self.bn1 = nn.BatchNorm2d(bottleneck_channels)

        self.conv2 = nn.Conv2d(in_channels=bottleneck_channels, out_channels=bottleneck_channels,
                               kernel_size=3, padding=1, bias=False, stride=stride)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes = 1000, n_segments=8):
        ...
        self.layer1 = self._make_layer(
            block, bottleneck_channels=64, out_channels=256, blocks=num_blocks[0], stride=1)
        self.layer2 = self._make_layer(
            block, bottleneck_channels=128, out_channels=512, blocks=num_blocks[1], stride=2)
        self.layer3 = self._make_layer(
            block, bottleneck_channels=256, out_channels=1024, blocks=num_blocks[2], stride=2)
        self.layer4 = self._make_layer(
            block, bottleneck_channels=512, out_channels=2048, blocks=num_blocks[3], stride=2)
